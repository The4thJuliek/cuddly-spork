...
if ( !checkif("cqp -v") ) {
	if ( confirm("Corpus WorkBench does not seem to be installed - do you want to install it?", 1) ) {
		...
		if ( $system eq 'debian/ubuntu') {
			`$installcmd autoconf bison flex gcc make pkg-config libc6-dev libncurses5 libncurses5-dev libpcre3-dev libglib2.0-0 libglib2.0-dev libreadline libreadline-dev`;
		};
